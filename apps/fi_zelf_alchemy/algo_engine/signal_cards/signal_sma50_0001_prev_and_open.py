...
def signal_sma50_0001_prev_and_open(db, signal_db, candle_formats, pair):
    ...
    record_proxy_1day = fetch_active_from_db(db, signal_db, pair, '1day', 'SMA50')
    record_proxy_1week = fetch_active_from_db(db, signal_db, pair, '1week', 'SMA50')
    record_proxy_1month = fetch_active_from_db(db, signal_db, pair, '1month', 'SMA50')
    ...
    if 'no_signal' in new_proxy:
        ...
        if int(new_proxy['sdp_0']) == int(record_proxy['sdp_0']):
            ...
            if int(new_proxy['sdp_0']) >= int(record_proxy['sdp_0']):
                ...
                sdp_1 = record_proxy['sdp_0']
            flag_signal_data = flag_signal(db, signal_db, record_proxy, int(sdp_1))
