...
def card_sma50_day_spot_SLmoving2prcnt(db, signal_db, trade_db, bank_db, signal_trade_db, pair, command):
    ...
    if flagged_trades_data:
        for data in flagged_trades_data:
            if data['trade_action'] == 'buy' and data['trade_status'] == 'filled':
                ...
                # PLACE STOP LOSS
                stop_price = data['price'] - (data['price'] * 0.02)  # Subtract 3% of the value
...
#MOVE STOP LOSS
    ...
    if placed_stoplimit_trades_data:
        # print(f'placed_stoplimit_trades_data {placed_stoplimit_trades_data}')
        # print(f'current  {fz_feeder_cycle_last_candle['close']}')
        # print(f'stop  {placed_stoplimit_trades_data[0]['trade_entry_stop']}')
        # print(f'time {fz_feeder_cycle_last_candle['time']}')
        # if 5% spread then trigger the move of stop loss 2% up
        if is_difference_at_least_n_percent(fz_feeder_cycle_last_candle['close'], placed_stoplimit_trades_data[0]['trade_entry_stop'], 5):
            # print(f'difference > 5% between {fz_feeder_cycle_last_candle['close']} and {placed_stoplimit_trades_data[0]['trade_entry_stop']}')
    ...
    if flagged_sma50_day_bull_signals:
        ...
        if stoplimit_trades_data:
            for data in stoplimit_trades_data:
                ...
                place_new_order_data = place_new_order_spot(db=db, signal_db=signal_db, trade_db=trade_db,
                                                            bank_db=bank_db, signal_trade_db=signal_trade_db,
                                                            trade_id=new_trade_id,
                                                            signal=flagged_sma50_day_bull_signals[0],
                                                            price=not_traded_sma50_bear_day_signals[0]['tp_entrance_1'],
                                                            amount_in=currency_sell_balance, trade_type='spot',
                                                            trade_position='long', trade_action='sell',
