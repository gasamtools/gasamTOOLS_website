...
def register_database(db, app):
    class fiZelfWorkhorseKeysDB(db.Model):
        __tablename__ = 'app_fi_zelf_workhorse_keys_db'
...
def json_logic(current_user, db, User, GasamApp, json_data, files_data):
    ...
    if json_data['js_function'] == 'fz_fetcher':
        ...
        return fz_fetcher(current_user, db, User, GasamApp, json_data, files_data, 'app_fi_zelf_workhorse_keys_db')


def app_ini(current_user, db, User, GasamApp, json_data, files_data):
    from main import check_and_install_requirements
    requirements_dir = os.path.join('apps', 'fi_zelf_workhorse',
                                    'fi_zelf_workhorse_requirements.txt')
